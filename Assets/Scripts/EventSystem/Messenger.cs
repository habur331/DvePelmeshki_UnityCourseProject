...
static internal class MessengerInternal {
	...
	static public void OnBroadcasting(string eventType, MessengerMode mode)
	{
		switch (mode)
		{
			case MessengerMode.REQUIRE_LISTENER when !eventTable.ContainsKey(eventType):
				throw new MessengerInternal.BroadcastException(string.Format("Broadcasting message {0} but no listener found.", eventType));
			case MessengerMode.DONT_REQUIRE_LISTENER when !eventTable.ContainsKey(eventType):
...
static public class Messenger { 
	...
	static public void Broadcast<TReturn>(string eventType, Action<TReturn> returnCall) {
 	...
 	static public void Broadcast(string eventType, MessengerMode mode) {
		...
		var invocationList = MessengerInternal.GetInvocationList<Action>(eventType) ?? new Action[]{};
